function [fhat,that] = reassignment_get_displacements_tmp(Sw,Stw,Sdw,torig,forig,dt)
...
Stw(idx) = real(Stw(idx)./Sw(idx))/dt;             % for time
Sdw(idx) = imag(Sdw(idx)./Sw(idx))/2/pi;             % for frequency
